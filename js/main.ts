...
class Utils {
	...
	static removeOptions(selectElement: HTMLSelectElement | null, end = 1) {
		/*
	...
	static getFromSelect(html_id: string): [number, string] {
		/* 
			Получить выбранный элемент в виде `индекс,текст`, из тега `select` по указному `html_id` 
		*/
		const elm: any = document.getElementById(html_id);
		if (elm) {
	...
	static getElementsByClassNameUp(elm: HTMLElement, req_class: string) {
		/*
		...
		*/
		...
		function _self(elm: any, req_class: string): HTMLElement | null {
...
class LogicHotKeyDict {
	static toStandard() {
		...
		for (let x of Object.keys(G_HotKeyDict)) {
			for (let x2 of Object.keys(G_HotKeyDict[x])) {
				...
				let capsEntries = entries.map((entry: any[]) => [
	...
	static _toStandardValue(text: string) {
...
class UserSelect {
	...
	static BuildSelectProgram() {
		...
		Utils.removeOptions(
			<HTMLSelectElement>document.getElementById("select-program")
		);
	...
	static BuildSelectPlace() {
		...
		Utils.removeOptions(
			<HTMLSelectElement>document.getElementById("select-place")
		);
		// Выбранная программа
...
class VirtualHotKey {
	...
	static ClearTakeKey() {
		...
		this.GetAllKey().forEach((elm) => {
			let elm_p: HTMLParamElement = <HTMLParamElement>elm.parentNode;
	...
	static _ClickKey(event: { target: any }) {
		/* 
	...
	static _ClickTakeNestedKey(elm: {
		parentNode: { classList: { toggle: (arg0: string) => void } };
		value: any;
	}) {
		/*
	...
	static _ClickTakeSimpleKey(elm: { value: any }) {
		/*
		...
		*/
		...
		for (let xw of ListHotKey.getAllHtmlElement()) {
			let rhk: HTMLInputElement = xw.querySelector(
				".radio_hot_key .change_hot_key"
			);
	...
	static _BuildMappingKeyFromHtmlKeyboard() {
		...
		*/
		...
		VirtualHotKey.GetAllKey().forEach((elm: HTMLInputElement) => {
...
class ParseHotKey {
	...
	static _BaseParse(text: string): string[] {
		/*
	...
	static toStandard(text: string) {
		return this.toSelfKeyboard(text).join("+");
	}
	static toSelfKeyboard(text: any) {
