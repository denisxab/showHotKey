class ListHotKey {
    ...
    static _Blur_change_hot_key(event) {
        ...
        if (elm.value != this.startFocusListHotKey) {
            ...
            const val_dict = G_HotKeyDict[Utils.getFromSelect("select-program")[1]][Utils.getFromSelect("select-place")[1]][this.startFocusListHotKey];
            // Удаляем комбинацию
            delete G_HotKeyDict[Utils.getFromSelect("select-program")[1]][Utils.getFromSelect("select-place")[1]][this.startFocusListHotKey];
            ...
            G_HotKeyDict[Utils.getFromSelect("select-program")[1]][Utils.getFromSelect("select-place")[1]][elm_value] = val_dict;
    ...
    static getAllHtmlElement() {
        return document.querySelectorAll("#info_down #list_hot_key .list_hot_key_radio");
    }
    static ClearList() {
    ...
    static AgainShowListHotKey() {
        ...
        */
        ...
        // Выбранная программа
        let select_program_arr = Utils.getFromSelect("select-program");
        let select_place_arr = Utils.getFromSelect("select-place");
        ...
        if (select_program_arr !== null && select_place_arr !== null) {
            ...
            const select_program = select_program_arr[1];
            const select_place = select_place_arr[1];
            ...
            for (let i = 0; i <= end; i++) {
                ...
                <div class="radio_hot_key">
					...
					<input type="text" class="change_hot_key" value="${ParseHotKey.toStandard(Object.keys(list_hot_key)[i])}">
                </div>
    ...
    static _ClickElmFomListHotKey(event) {
        ...
        const elm = Utils.getElementsByClassNameUp(event.target, "list_hot_key_radio");
...
ListHotKey.lastSelectElement = undefined;
// Хранит текст комбинации клавиш на момент фокуса
ListHotKey.startFocusListHotKey = undefined;
